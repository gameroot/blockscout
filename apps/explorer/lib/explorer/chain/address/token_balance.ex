defmodule Explorer.Chain.Address.TokenBalance do
  ...
  The Token Holders are the addresses that have some balance in the Token. So this query is
  ...
  * Excluding the burn address (0x0000000000000000000000000000000000000000).
  ...
  defp token_holders_query(contract_address_hash) do
    ...
    from(
      ...
      where:
        tb.token_contract_address_hash == ^contract_address_hash and tb.value > 0 and
          tb.address_hash != ^burn_address_hash,
      order_by: [desc: :block_number]
    )
  end
  ...
  defp page_token_balances(query, %PagingOptions{key: value}) do
    where(query, [token_balance], token_balance.value < ^value)
  end
end
