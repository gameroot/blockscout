defmodule Explorer.Chain.Token do
  ...
  * `:contract_address_hash` - Address hash foreign key
  ...
  """
  @spec with_transfers_by_address(Hash.t(), [paging_options()]) :: []
  ...
  def page_token(query, %PagingOptions{key: {name, type}}) do
    where(
      query,
      [token],
      token.type < ^type or (token.type == ^type and token.name > ^name)
    )
  end
end
